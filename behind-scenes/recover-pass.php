...
if(isset($_POST['reset_email_submit']))
{
    ...
    if(preg_match('%^[A-Za-z0-9._\%-]+\@[A-Za-z0-9.-]+\.[A-Za-z]{2,5}$%',stripslashes(trim($_POST['resetpass_email']))))
        $recovery_email = clean_input($_POST['resetpass_email']);
    else
    {
    ...
    if($recovery_email)
    {
        ...
        if(mysqli_affected_rows($conn) == 1)
        {
            ...
            if(mysqli_affected_rows($conn) == 1)
            {
                ...
                $access_link = "http://www.almasparlour.co.ke/almas-parlour/view/reset.php?rid=".$user_id."&ra=$pass_reset_key";
